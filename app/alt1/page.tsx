...
const imgs = [human1, human2, human3, human4, human5, human6, human7, human8, human9, human10, human11, human12, human13, human14, human15, human16, human17, human18, human23, human24,
...
export default function Home() {
  ...
  return (
    <main className={`h-[${height*2}px] w-[${width*2}px] overflow-hidden`}>
      
      <h1 className={`font-arial font-normal text-7xl text-center w-[35rem] h-fit fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2`}>Things to believe in: a manifesto.</h1>
      
      <p className="left-1/2 -translate-x-1/2 text-center fixed top-4 font-arial uppercase font-normal text-md z-50">Believe</p>
      <p className="left-1/2 -translate-x-1/2 rotate-180 text-center fixed bottom-4 font-arial uppercase font-normal text-md z-50">Question</p>
      <p className="top-1/2 -translate-y-1/2 -rotate-90  text-center fixed -left-0 font-arial uppercase font-normal text-md z-50">Practice</p>
      <p className="top-1/2 -translate-y-1/2 rotate-90 text-center fixed -right-0 font-arial uppercase font-normal text-md z-50">Practice</p>

        {
          imgs.map((image: StaticImageData, index: number) =>
            (
              <Draggable key={index} defaultPosition={{x: getRandom(16, width*2-500), y: getRandom(16, height*2-500)}}>
                <div className={`overflow-hidden absolute cursor-move`}>
                  <Image src={image} alt='' className='w-60 h-auto pointer-events-none'/>
                </div>
              </Draggable>
            )
          )
